<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:88:"/www/wwwroot/125.maoerle.cn/www_fuda/public/../application/admin/view/deposit/index.html";i:1568109530;s:86:"/www/wwwroot/125.maoerle.cn/www_fuda/application/admin/view/layouts/layout_iframe.html";i:1568109530;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<nav class="breadcrumb">
	...
	<span class="c-gray en">&gt;</span> 交易保证金
...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray">
		<span class="l">
			<?php if(in_array('admin/Deposit/remove', \think\Session::get('ACCESS_LIST'))): ?>
			<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius">
				<i class="Hui-iconfont Hui-iconfont-del3"></i> 批量删除
			</a>
			<?php endif; if(in_array('admin/Deposit/create', \think\Session::get('ACCESS_LIST'))): ?>
			<a class="btn btn-primary radius" href="javascript:;" onclick="deposit_add('添加保证金','<?php echo url("admin/Deposit/create"); ?>','','350')">
	...
	<table class="table table-border table-bordered table-hover table-bg">
		...
		<tr class="text-c mode-lists">
			...
			<td class="f-14">
				<?php if(in_array('admin/Deposit/modify', \think\Session::get('ACCESS_LIST'))): ?>
				<input class="btn btn-primary size-MINI radius" type="button" title="编辑保证金" onclick="deposit_edit('编辑保证金','<?php echo url("admin/Deposit/modify", ['id' => $item['id']]); ?>','','350')" value="编辑">
				<?php endif; if(in_array('admin/Deposit/remove', \think\Session::get('ACCESS_LIST'))): ?>
				<input class="btn btn-danger size-MINI radius" type="button" title="删除保证金" onclick="deposit_del(this, <?php echo $item['id']; ?>);" value="删除">
				<?php endif; ?>
			</td>
		</tr>
...
<script>
    /*用户-添加*/
    <?php if(in_array('admin/Deposit/create', \think\Session::get('ACCESS_LIST'))): ?>
    function deposit_add(title,url,w,h){
        layer_show(title,url,w,h);
    }
	<?php endif; if(in_array('admin/Deposit/modify', \think\Session::get('ACCESS_LIST'))): ?>
    function deposit_edit(title,url,w,h){
        layer_show(title,url,w,h);
    }
	<?php endif; if(in_array('admin/Deposit/remove', \think\Session::get('ACCESS_LIST'))): ?>
    function deposit_del(_obj, _id) {
        layer.confirm('确认要删除保证金吗？',function(index){
            var _oData = {id:_id},
                _url = '<?php echo url("admin/Deposit/remove", ["act" => "single"]); ?>',
    ...
    function datadel()
    {
        ...
        if(_ids.length == 0){
            ...
            layer.confirm('确认要删除保证金吗？',function(index){
                var _oData = {ids: _ids},
                    _url = '<?php echo url("admin/Deposit/remove", ["act" => "patch"]); ?>',
                    _func = function (_resp) {
                        layer.close(index);
                        if (!_resp.state) {
