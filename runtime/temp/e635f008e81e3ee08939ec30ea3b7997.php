<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:86:"/www/wwwroot/125.maoerle.cn/www_fuda/public/../application/admin/view/lever/index.html";i:1568109530;s:86:"/www/wwwroot/125.maoerle.cn/www_fuda/application/admin/view/layouts/layout_iframe.html";i:1568109530;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray">
		<span class="l">
			<?php if(in_array('admin/Lever/remove', \think\Session::get('ACCESS_LIST'))): ?>
			<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius">
				<i class="Hui-iconfont Hui-iconfont-del3"></i> 批量删除
			</a>
			<?php endif; if(in_array('admin/Lever/create', \think\Session::get('ACCESS_LIST'))): ?>
			<a class="btn btn-primary radius" href="javascript:;" onclick="lever_add('添加杠杆','<?php echo url("admin/Lever/create"); ?>','','350')">
	...
	<table class="table table-border table-bordered table-hover table-bg">
		...
		<tr class="text-c mode-lists">
			...
			<td class="f-14">
				<?php if(in_array('admin/Lever/modify', \think\Session::get('ACCESS_LIST'))): ?>
				<input class="btn btn-primary size-MINI radius" type="button" title="编辑杠杆" onclick="lever_edit('编辑杠杆','<?php echo url("admin/Lever/modify", ['id' => $item['id']]); ?>','','350')" value="编辑">
				<?php endif; if(in_array('admin/Lever/remove', \think\Session::get('ACCESS_LIST'))): ?>
				<input class="btn btn-danger size-MINI radius" type="button" title="删除杠杆" onclick="lever_del(this, <?php echo $item['id']; ?>);" value="删除">
				<?php endif; ?>
			</td>
		</tr>
...
<script>
    /*用户-添加*/
    <?php if(in_array('admin/Lever/create', \think\Session::get('ACCESS_LIST'))): ?>
    function lever_add(title,url,w,h){
        layer_show(title,url,w,h);
    }
	<?php endif; if(in_array('admin/Lever/modify', \think\Session::get('ACCESS_LIST'))): ?>
    ...
    function lever_del(_obj, _id) {
        layer.confirm('确认要删除此杠杆配置吗？',function(index){
            var _oData = {id:_id},
                _url = '<?php echo url("admin/Lever/remove", ["act" => "single"]); ?>',
    ...
    function datadel()
    {
        ...
        if(_ids.length == 0){
            ...
            layer.confirm('确认要删除此杠杆配置吗？',function(index){
                var _oData = {ids: _ids},
                    _url = '<?php echo url("admin/Lever/remove", ["act" => "patch"]); ?>',
                    _func = function (_resp) {
                        layer.close(index);
                        if (!_resp.state) {
