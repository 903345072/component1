<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:86:"/www/wwwroot/125.maoerle.cn/www_fuda/public/../application/admin/view/team/member.html";i:1568109530;s:86:"/www/wwwroot/125.maoerle.cn/www_fuda/application/admin/view/layouts/layout_iframe.html";i:1568109530;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<div class="page-container">
    ...
    <div class="text-c">
        ...
        <input type="text" class="input-text" style="width:170px;" placeholder="上级运营中心" name="operate" value="<?php echo (isset($search['operate']) && ($search['operate'] !== '')?$search['operate']:''); ?>">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
            ...
            <?php if(in_array('admin/Team/createMember', \think\Session::get('ACCESS_LIST'))): ?>
			<a class="btn btn-primary radius" href="javascript:;" onclick="admin_add('添加会员','<?php echo url("admin/Team/createMember"); ?>','', '450')">
    ...
    <table class="table table-border table-bordered table-hover table-bg">
        ...
        <tr class="text-c admin-lists">
            ...
            <td class="f-14">
                <?php if(in_array('admin/Team/modifyMember', \think\Session::get('ACCESS_LIST'))): ?>
                <input class="btn btn-success size-MINI radius" type="button" title="编辑会员" onclick="admin_edit('编辑会员','<?php echo url("admin/Team/modifyMember", ['id' => $item['admin_id']]); ?>','','450')" value="修改">
                <?php endif; if(in_array('admin/Team/memberWechat', \think\Session::get('ACCESS_LIST'))): ?>
                <input class="btn btn-secondary size-MINI radius" type="button" title="微信公众号" onclick="admin_wechat('<?php echo $item['username']; ?>-微信公众号','<?php echo url("admin/Team/memberWechat", ['id' => $item['admin_id']]); ?>');" value="微信">
                <?php endif; if(in_array('admin/Team/memberPoint', \think\Session::get('ACCESS_LIST'))): ?>
                <input class="btn btn-primary size-MINI radius" type="button" title="返点修改" onclick="member_point('返点修改','<?php echo url("admin/Team/memberPoint", ['id' => $item['admin_id']]); ?>','','380');" value="返点">
...
<script>
    ...
    <?php endif; if(in_array('admin/Team/modifyMember', \think\Session::get('ACCESS_LIST'))): ?>
    function admin_edit(title,url,w,h){
        layer_show(title,url,w,h);
    }
    <?php endif; if(in_array('admin/Team/memberPoint', \think\Session::get('ACCESS_LIST'))): ?>
    function member_point(title,url,w,h){
        layer_show(title,url,w,h);
    }
    <?php endif; if(in_array('admin/Team/memberWechat', \think\Session::get('ACCESS_LIST'))): ?>
    function admin_wechat(title,url){
