<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:93:"/www/wwwroot/125.maoerle.cn/www_fuda/public/../application/admin/view/manager/auditLists.html";i:1568109530;s:86:"/www/wwwroot/125.maoerle.cn/www_fuda/application/admin/view/layouts/layout_iframe.html";i:1568109530;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<div class="page-container">
    ...
    <table class="table table-border table-bordered table-hover table-bg">
        ...
        <tr class="text-c admin-lists">
            ...
            <td class="f-14">
                <?php if($item['state'] == 0): if(in_array('admin/Manager/audit', \think\Session::get('ACCESS_LIST'))): ?>
                <input class="btn btn-success size-MINI radius" type="button" title="审核通过" onclick="admin_success('<?php echo $item['id']; ?>', '1', '<?php echo $item['user_id']; ?>')" value="通过">
                <input class="btn btn-danger size-MINI radius" type="button" title="审核拒绝" onclick="admin_refuse('<?php echo $item['id']; ?>', '2', '<?php echo $item['user_id']; ?>')" value="拒绝">
                <?php endif; endif; ?>
            </td>
        </tr>
...
<script>
    <?php if(in_array('admin/Manager/audit', \think\Session::get('ACCESS_LIST'))): ?>
    function admin_success(_id, state, uid)
    {
        layer.confirm('是否通过审核?', {icon: 1, title:'提示'}, function(index){
            ...
            var _oData = {id : _id, state:state, user_id:uid},
                _url = '<?php echo url("admin/Manager/audit"); ?>',
