<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:84:"/www/wwwroot/125.maoerle.cn/www_fuda/public/../application/admin/view/ai/stocks.html";i:1568109530;s:86:"/www/wwwroot/125.maoerle.cn/www_fuda/application/admin/view/layouts/layout_iframe.html";i:1568109530;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<nav class="breadcrumb">
	...
	<?php if(!(empty($type_name) || (($type_name instanceof \think\Collection || $type_name instanceof \think\Paginator ) && $type_name->isEmpty()))): ?><span class="c-gray en">&gt;</span> <?php echo $type_name; endif; ?>
...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray">
		<span class="l">
			<?php if(in_array('admin/Ai/remove', \think\Session::get('ACCESS_LIST'))): ?>
			<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius">
				<i class="Hui-iconfont Hui-iconfont-del3"></i> 批量删除
			</a>
			<?php endif; if(in_array('admin/Ai/create', \think\Session::get('ACCESS_LIST'))): ?>
			<a class="btn btn-primary radius" href="javascript:;" onclick="stock_add('添加股票','<?php echo url("admin/Ai/create"); ?>','','450')">
	...
	<table class="table table-border table-bordered table-hover table-bg">
		...
		<tr class="text-c mode-lists">
			...
			<!--<td><?php echo $item['full_code']; ?></td>-->
			<td><?php echo number_format($item['income'],2); ?></td>
			...
			<td class="f-14">
				<?php if(in_array('admin/Ai/modify', \think\Session::get('ACCESS_LIST'))): ?>
				<input class="btn btn-primary size-MINI radius" type="button" title="编辑股票" onclick="stock_edit('编辑股票','<?php echo url("admin/Ai/modify", ['id' => $item['id']]); ?>','','450')" value="编辑">
				<?php endif; if(in_array('admin/Ai/remove', \think\Session::get('ACCESS_LIST'))): ?>
				<input class="btn btn-danger size-MINI radius" type="button" title="股票删除" onclick="stock_del(this, <?php echo $item['id']; ?>);" value="删除">
				<?php endif; ?>
			</td>
		</tr>
...
<script>
    /*用户-添加*/
    <?php if(in_array('admin/Ai/create', \think\Session::get('ACCESS_LIST'))): ?>
    function stock_add(title,url,w,h){
        var func = function(layero, index){
            var body = layer.getChildFrame('body', index);
            body.contents().find("input[name='type_id']").val(<?php echo $type_id; ?>);
        };
        layer_show_2(title,url,w,h,func);
    }
	<?php endif; if(in_array('admin/Ai/modify', \think\Session::get('ACCESS_LIST'))): ?>
    ...
    function stock_del(_obj, _id) {
        layer.confirm('确认要删除推荐股票吗？',function(index){
            var _oData = {id:_id},
                _url = '<?php echo url("admin/Ai/remove", ["act" => "single"]); ?>',
    ...
    function datadel()
    {
        ...
        if(_ids.length == 0){
            ...
            layer.confirm('确认要删除推荐股票吗？',function(index){
                var _oData = {ids: _ids},
                    _url = '<?php echo url("admin/Ai/remove", ["act" => "patch"]); ?>',
                    _func = function (_resp) {
                        layer.close(index);
                        if (!_resp.state) {
