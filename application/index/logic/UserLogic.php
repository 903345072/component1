...
class UserLogic
{
    ...
    // 意见反馈
    public function saveIdea($userId, $data)
    {
        $user = User::find($userId);
        $data['user_id'] = $userId;
        $data['idea_name'] = $data['idea_name'];
        $data['idea_mobile'] = $data['idea_mobile'];
        $data['idea_content'] = $data['idea_content'];
        $data['create_time'] = time();
        $res = User_idea::insert($data);
    ...
    public function pageUserOrder($userId, $state = 1, $field = "*", $pageSize = 4){
        try{
            ...
            $res = Order::with("belongsToMode")->where($where)->order('create_at desc')->field($field)->paginate($pageSize);
