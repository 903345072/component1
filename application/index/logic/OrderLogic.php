...
class OrderLogic
{
    ...
    public function getCodesBy($where=[])
    {
        ...
        return Order::where($map)->limit(18)->column('code');
    ...
    public function orderByState($state = 1)
    {
        ...
        $where["create_at"] = ['LT',strtotime(date('Y-m-d'.'00:00:00',time()))];
    ...
    public function forceSell($orderId, $sellPrice)
    {
        ...
        try{
            ...
            $pro1 = $profit + $order['deposit']*0.7;
            if($pro1 < 0){  //爆仓=>最多扣除保证金*0.7
                $pro1 = -$order['deposit']*0.7;
            }else{
                $pro1 = $profit;
            }
            $data = [
            ...
            if($profit > 0){
                ...
                // 用户资金
                $user = User::find($order['user_id']);
                $s = $profit + $order['deposit']*0.7;
                if($s < 0){  //爆仓=>最多扣除保证金*0.7
                    $account = $order['deposit']*0.3;
                }else{
                    $account = $profit + $order['deposit'];
                }
