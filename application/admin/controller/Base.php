...
//            $this->error('您没有权限访问');
    ...
    //             ->setWriterByName('png');
    //         if(!file_exists('./upload/manager_pidcode/')){
    //             mkdir('./upload/manager_pidcode/');
    //         }
    //         $qrCode->writeFile('./upload/manager_pidcode/' . $uid . '.png');
    //     }
    // }
    // 代理商二维码
    public function createManagerQrcode01($admin_id)
    {
        if($admin_id > 0) {
            $AdminInfo = (new AdminLogic())->adminById($admin_id);
            $qrCode = new QrCode();
            //想显示在二维码中的文字内容，这里设置了一个查看文章的地址
            $url = url('index/Home/register', ["admin_id" => $admin_id,"code"=>$AdminInfo['code']], true, true);
            $qrCode->setText($url)
                ->setSize(300)
                ->setForegroundColor(array('r' => 0, 'g' => 0, 'b' => 0, 'a' => 0))
                ->setBackgroundColor(array('r' => 255, 'g' => 255, 'b' => 255, 'a' => 0))
                // ->setBackgroundColor(array('r' => 255, 'g' => 0, 'b' => 0, 'a' => 0))
                // ->setLabel('经纪人：' . $AdminInfo['username'], '.')
                ->setLabelFontSize(16)
                // ->setLogoPath($_SERVER['DOCUMENT_ROOT'] . trim($AdminInfo['face']))
                ->setWriterByName('png');
            if(!file_exists('./upload/manager_pidcode/')){
                mkdir('./upload/manager_pidcode/');
            }
            $qrCode->writeFile('./upload/manager_pidcode/' . $admin_id . '.png');
        }
    }
}