...
class Order extends Base
{
    ...
    public function index()
    {
        ...
        $jiancang_fee = array_sum(collection($_res['lists']['data'])->column("jiancang_fee"));
        $defer = array_sum(collection($_res['lists']['data'])->column("defer"));
        $profit = array_sum(collection($_res['lists']['data'])->column("profit"));
        ...
        $pc_sum['profit_sum'] = array_sum(collection($_res['hz_sum'])->column("profit"));  
        $pc_sum['defer_sum'] = array_sum(collection($_res['hz_sum'])->column("defer"));  
        $pc_sum['jiancang_fee_sum'] = array_sum(collection($_res['hz_sum'])->column("jiancang_fee"));  

        $this->assign("defer", $defer);
        $this->assign("jiancang_fee", $jiancang_fee);
    ...
    public function history()
    {
        ...
        $jiancang_fee = array_sum(collection($_res['lists']['data'])->column("jiancang_fee"));
        $totel_hands = array_sum(collection($_res['lists']['data'])->column("sell_hand"));
        $defer = array_sum(collection($_res['lists']['data'])->column("defer"));
        $profit = array_sum(collection($_res['lists']['data'])->column("profit"));
        ...
        $this->assign("datas", $_res['lists']);
        $this->assign("pages", $_res['pages']);
        $this->assign("totel_hands", $totel_hands);
        $this->assign("defer", $defer);
        $this->assign("jiancang_fee", $jiancang_fee);
        $this->assign("profit", $profit);
        $this->assign("pc_sum", $pc_sum);
    ...
    public function position()
    {
        ...
        if($_res['lists']['data']){
            ...
            array_filter($_res['lists']['data'], function(&$item) use ($quotation){
                ...
                $item['pl'] = isset($quotation[$item['code']]['last_px']) ? round(($item['last_px'] - $item['price']) * $item['hand'],2) : "-";
            });
        }
        foreach ($_res['hz_sum'] as $key => $value) {
            $_res['hz_sum'][$key] = $value = $value->toArray();
        ...
        if($_res['hz_sum']){
            $codes = array_column($_res['hz_sum'], "code");
            $quotation = (new StockLogic())->stockQuotationBySina($codes);
            array_filter($_res['hz_sum'], function(&$item) use ($quotation){
                $item['last_px'] = isset($quotation[$item['code']]['last_px']) ? number_format($quotation[$item['code']]['last_px'], 2) : '-';
                $item['pl'] = isset($quotation[$item['code']]['last_px']) ? round(($item['last_px'] - $item['price']) * $item['hand'],2) : "-";
            });
        }

        $pc_sum['profit_sum'] = array_sum(collection($_res['hz_sum'])->column("pl"));  
        $pc_sum['defer_sum'] = array_sum(collection($_res['hz_sum'])->column("defer"));  
        $pc_sum['jiancang_fee_sum'] = array_sum(collection($_res['hz_sum'])->column("jiancang_fee"));  

        $jiancang_fee = array_sum(collection($_res['lists']['data'])->column("jiancang_fee"));
        $totel_hands = array_sum(collection($_res['lists']['data'])->column("hand"));
        $defer = array_sum(collection($_res['lists']['data'])->column("defer"));
        $stop_profit_price = array_sum(collection($_res['lists']['data'])->column("stop_profit_price"));
        $stop_loss_price = array_sum(collection($_res['lists']['data'])->column("stop_loss_pric"));
        $profit = array_sum(collection($_res['lists']['data'])->column("pl"));
        $this->assign("totel_hands", $totel_hands);
        $this->assign("defer", $defer);
        $this->assign("jiancang_fee", $jiancang_fee);
        $this->assign("profit", $profit);
        $this->assign("datas", $_res['lists']);
        $this->assign("pages", $_res['pages']);
        $this->assign("stop_profit_price", $stop_profit_price);
        $this->assign("stop_loss_price", $stop_loss_price);
        $this->assign("pc_sum", $pc_sum);
        $this->assign("search", input(""));
        return view();
    }
