		...
		$.ajax({
			...
			success:function(res){
                ...
                if(res==null||res==''||res.length==0){
                    ...
                    for( var k in res ){
                        html+='<tr><td>'+res[k].prod_name+'</td><td>'+res[k].code+'</td><td>'+res[k].pinyin+'</td></tr>';
                    }
                }
                tbody.html(html);
                //3.cue框显示
...
function getTimeLine(){
	$.post("stock/real", {code:selectedCode}, function(data){
		// if(data.showapi_res_code != '0' || data.showapi_res_body.ret_code != '0'){
		// 	return;
        // }
        if(data.state == 1){
            var arr = data.data.trend;
            ...
            var yestclose = data.data.preclose_px;
			//yestclose = "25.11"，昨天收盘价
			//dataList是几百个二维数组，name是时间，valu是数组，包括【0】时间和【1】价格
			//绘制分时图
...
function getStockInfo(){
	...
	$.post("stock/info", {code:selectedCode}, function(data){
		if(data.state != 1){
			return;
        }
        calcNum();
...
function initKChart(){
    $.post("stock/kline", {code:selectedCode}, function(res){
        var data = res.data;
        if(data.showapi_res_code != 0 || data.showapi_res_body.ret_code != 0){
            return;
        }
...
$("#buy_number").off('keyup').on('keyup',function(){
	var price = parseInt($('#buy_number').val());
	if($("#buy_number").val()==''||$('#buy_number').val()=='0'){price=1000}
	if(price>50000){$("#buy_number").val('50000');price=50000;}
	//金额高亮
...
//	console.log(price);
    // $("#check-surplus_ul>li").html(price * 5000);
    // $("#stop-loss_ul>li:eq(0)").html(price * -1000);
    // $("#>stop-loss_ulli:eq(1)").html(price * -1333);
    // $("#stop-loss_ul>li:eq(2)").html(price * -1700);
    $("#publicFee").html(price * publicFee );
    $("#delay_fee").html(price * delayFee);
...
$("#buy_celue_ul").on("click" , "li" , function(){
